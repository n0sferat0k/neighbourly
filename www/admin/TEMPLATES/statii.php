...
<center>
	<form action="" method="post" enctype="multipart/form-data">
		...
		<div class="PAGE">
			<div class="PAGE_LEFT">	
				<a href="" class="navlink">&laquo; Inapoi</a>
				<?php if($has_add_numerics) { ?>				
					<?php for($add_numerics_counter=0;$add_numerics_counter < count($add_numerics);$add_numerics_counter ++) { switch($add_numerics[$add_numerics_counter]['case']){
			  			case 'editable': ?>
					    	<div class="LABLE">	
					  	...
					  	case 'static': ?>
					  		<!--
						    	...
						    	<div class="POSITIONER">-->
									<input type="hidden" name="<?php echo $add_numerics[$add_numerics_counter]['add_numericsINP']; ?>" value="<?php echo $add_numerics[$add_numerics_counter]['add_numericsVAL']; ?>" class="INPUT_TEXT" readonly="readonly"/>
								<!--</div>-->
				...
				<?php if($has_add_strings) { ?>
					<?php for($add_strings_counter=0;$add_strings_counter < count($add_strings);$add_strings_counter ++) { switch($add_strings[$add_strings_counter]['case']){
			  			case 'editable': ?>
					    	<div class="LABLE">	
					  	...
					  	case 'static': ?>
					  		<!--
						    	...
						    	<div class="POSITIONER">-->
									<input type="hidden" name="<?php echo $add_strings[$add_strings_counter]['add_stringsINP']; ?>" value="<?php echo $add_strings[$add_strings_counter]['add_stringsVAL']; ?>" class="INPUT_TEXT" readonly="readonly"/>
								<!--</div>-->
				...
				<?php if($has_accent) { ?>
					...
					<input style="float:left" type="checkbox" name="<?php echo $accentcheckINP; ?>" <?php if($is_accented) { ?> checked="checked" <?php } else { ?><?php } ?>/>
				...
				<?php if($has_zindex) { ?>
					...
					<div class="POSITIONER">
						<select name="<?php echo $zindexselectINP; ?>" class="INPUT_SELECT">
							<?php for($zindexes_counter=0;$zindexes_counter < count($zindexes);$zindexes_counter ++) { ?>
								<option value="<?php echo $zindexes[$zindexes_counter]['val']; ?>" <?php for($selected_counter=0;$selected_counter < count($zindexes[$zindexes_counter]['selected']);$selected_counter ++) { ?> selected="selected" <?php } ?> >
								...
								<div class="FRAME_INNER_POS2">
									<?php if($has_pic2) { ?>																					
										<?php if($multipic) { ?>											
											<div class="FRAME_INNER_<?php echo $VIEW_HIDE_IMGS; ?>" id="IMG_view" name="containers">
												...
												<?php for($images_counter=0;$images_counter < count($images);$images_counter ++) { ?>	
									...
									<?php if($has_file2) { ?>
										<div class="FRAME_INNER_<?php echo $VIEW_HIDE_FILES; ?>" id="FILE_view" name="containers">
											...
											<?php for($files_counter=0;$files_counter < count($files);$files_counter ++) { ?>
