...
class menuWrapper //Text Title Date Image
{
	private $params;
	private $assoc;
	private $parent = -1;
	...
	function add($obj)
	{
	 	$cnt = count($this->assoc);
		$this->assoc[$cnt] = $obj;
		$obj->setParent($this);		
	}
	function contains($object_key)
	{
	 	for($i=0;$i<count($this->assoc);$i++)
	 	{
			$par = $this->assoc[$i]->getParams();
			if($par['KEY'] == $object_key)
				return true;
		}
		return false;
	}
	function displayMenu($key)
	{
		if($this->params['DISPLAYMENUTAB']==true)
		{
			...
			if(($this->params['KEY']==$key)||($this->contains($key)))
	...
	function languageSelection($key)
	{
		if($this->params['KEY']==$key)
		{
		 	...
		 	if(isRequestParamEqualTo('param1', "setlang"))
		 	{
	...
	{	 
	 	if($this->params['KEY']==$key)
	 	{
			for($i=0;$i<count($this->assoc);$i++)
			{
			 	$p = $this->assoc[$i]->getParams();
			 	$p['DISPLAYMENUTAB'] = true;
                $p['MENU_template'] = "TEMPLATES/menutab2.php";			 	
			 	$this->assoc[$i]->setParams($p);
				$this->assoc[$i]->displayMenu($key);
			}
		}
	}
