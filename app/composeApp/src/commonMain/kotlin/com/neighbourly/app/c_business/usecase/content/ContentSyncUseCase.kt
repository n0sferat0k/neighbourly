        ...
        token?.let {
            ...
            if (force || (currentTs - lastSyncTs) > SYNC_DEBOUNCE_S) {
                ...
                if (lastSyncTs > 0 && newIds.isNotEmpty()) {
                    ...
                    val newItemsHouseholds =
                        dbInteractor.filterHouseholds(newItems.map { it.householdId }
                            .filterNotNull())

                    newItems.forEach { item ->
                        val houseName =
                            newItemsHouseholds.firstOrNull { it.householdid == item.householdId }?.name.orEmpty()
